...
require 'tmpdir'
require 'fileutils'
require 'smart_proxy_ansible_director/container_registry'
...
class PodmanAuthTest < Test::Unit::TestCase
  ...
  def test_setup_cert_dir_creates_temp_dir
    dir = Proxy::AnsibleDirector::ContainerRegistry::PodmanAuth.setup_cert_dir
    assert Dir.exist?(dir)
  ensure
    Proxy::AnsibleDirector::ContainerRegistry::PodmanAuth.cleanup(dir)
  end
  ...
  def test_cleanup_ignores_nil
    assert_nothing_raised { Proxy::AnsibleDirector::ContainerRegistry::PodmanAuth.cleanup(nil) }
  end

  def test_tls_args_includes_cert_dir
    args = Proxy::AnsibleDirector::ContainerRegistry::PodmanAuth.tls_args('/some/path')

    assert_match(/--tls-verify=true/, args)
    assert_match(%r{--cert-dir /some/path}, args)
  end

  def test_tls_args_escapes_spaces_in_path
    args = Proxy::AnsibleDirector::ContainerRegistry::PodmanAuth.tls_args('/some/path with spaces')

    assert_match(/--cert-dir /, args)
